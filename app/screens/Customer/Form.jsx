...
export default function Form({ route, navigation }) {
  ...
  const autofillProfile = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get("http://UF-MacBook-Pro.local:3001/profiles/get-profile2", { params: { email: CustomerEmail } });
      const p = data.user;
  ...
  const submitPersonalDetails = async () => {
    ...
    try {
      setLoading(true);
      await axios.post("http://UF-MacBook-Pro.local:3001/orders/place-order2", { full_name: fullName, address, phone, CustomerEmail, tailorEmail, orderId });
      setLoading(false);
      setStage("loading");
    } catch (error) {
      setLoading(false);
